...
- **下面所有数字量在 `3454a97`**（功能改动的最后一跳），并已在分支尖端复跑确认逐格相同。
  `3454a97` 之后只有两跳：一次文档重写，以及一次去掉注释行尾空格的 `style` 提交
  （`git diff 3454a97 HEAD -- plugins/` 只有一行，且仅差一个空格）。
  我不在正文里钉「交付提交 SHA」：每写一次文档 HEAD 就变一次，钉了必然过期。
...
| dsh-fleet | PASS | 182 | 0 | 1 | 183 |
| frontend-verify | PASS | 365 | 0 | 1 | 366 |
| selftest | PASS | 45 | 0 | 0 | 45 |
...
**上表是 `3454a97` 的数字。** 此前的 `d60ae14` 是 1054 / 0 / 8（tests 1062）；
`3454a97` 修掉了独立复核抓出的三条（见第 9 节），随之 +9 项测试。
两次运行都跑在交付树的最终形态上，退出码均为 0。
...
| 闸 | 基线 minPass | 交付 minPass | 变化 |
|---|---|---|---|
| dsh-agos | 172 | 196 | +24 |
| dsh-agos-router | 124 | 156 | +32 |
| dsh-fleet | 148 | 182 | +34 |
| selftest | 37 | 45 | +8 |
| cn-capabilities | 114 | 115 | +1 |
| dsh-mcp-bridge | 4 | 4 | 0 |
| frontend-verify | 365 | 365 | 0 |

**无一下降。** 这是**收紧**：原来 +24/+32/+34 的余量等于允许删掉那么多测试而门仍然绿，
正是下限机制要堵的洞。

⚠️ 这张表**重算过一次**。`d60ae14` 时是 194/156/176/44；`3454a97` 新增 9 项测试后
重算为 196/156/182/45。重算不是自动的——第一次我用了裸 `--write-floors`，
它被静默忽略，下限一字未动而我以为已上调（见 8.6）。现在裸标志会退 78。
正确写法：`--write-floors=scripts/acceptance/expected-counts.json`。

---

## 8. 验证过程中新暴露并已修的六个「测试测的是替身」

这六个不在任务书的八项里。前三个是跑全量验证时被咬出来的；
**8.4 / 8.5 是独立复核抓出来的，8.6 是我自己踩的**——三者都已修并有否证。
它们共享同一个病根：断言只覆盖了真实失败面的一部分维度，于是「绿」不等于「对」。
...
### 8.4 `session/list` 内调的信封形状仍是错的（独立复核抓出）

**这条最值得读。** 任务书第 6 项我改对了方法名（点号 → 斜杠），并写了 13 项测试，
全绿，我据此在文档里宣布该项完成。**方法名对了，信封形状还是错的**：
发的是 `payload: {}`，缺整个 `args` 层。固定宿主 `gateway/src/index.ts:950-953`
要求 payload 含**恰好一个** plain-object `args` 字段，`assertExactArguments`（:1112-1137）
再要求 args 的键与 descriptor 对上。所以 web profile 上的删除**照样永久 503**，
只是换了个失败理由——我修掉的是症状的一半。

正确形状是 `payload: { args: { _request: {} } }`，三处独立证据一致：本仓
`frontend/src/api-client/index.ts:71` 的 `ARG_KEY` 映射、同文件 `:131` 的
`payload: { args }` 信封构造、宿主 e2e 的 `remoteRpc(..., 'session/list', { _request: {} })`。

**我的 13 项测试为什么全绿**：它们的假 fetch 只被断言了 `url` 和 `method`，
请求体的其余部分根本没人看。测试对准了「方法名」这一个维度，而真实失败面还有
「信封形状」这一维——这与本节前三条是同一个病：**测试测的是替身**。
上一轮 `host.describe` 能腐烂一整轮没人发现，根因也是这个。

修法不只是改形状，而是让假 fetch 变成**会校验的假 gateway**：逐字实现宿主那两条谓词，
拒绝时照抄宿主行为（HTTP **200** + `ok:false`，不是 4xx）。再配一条**元证明负控**：
把四种错形状（缺 args、args 非对象、键名写成 `request`、payload 多带字段）
喂给同一个假 gateway，断言它真的会拒——否则「形状对了」这条测试永远绿，等于没测。

否证：`payload` 退回 `{}` 变红；键名改成 `request` 变红。

### 8.5 Fleet 40KB 预览把拒绝吞成空的 200

`lib/index.js` 的预览路径管进 `head -c 40000`，管道退出码是 **head 的**，
远端 `exit 9`（拒绝）传不出来。**不泄漏根外内容**（字节那趟本来就绑描述符），
但操作者分不清「产物是空的」和「产物被安全守卫拒了」。这条是 Fleet 实施者交回时
点名的，`index.js` 属我所有权，我上一跳漏了没接。

改成单次远端执行 + 判决行先到再决定状态码；`limitBytes` 在已持有的 fd 上截断
（`head -c N <&3`），是同一对象的更小一次读，不是第二次解析名字。

新增 6 项测试走**真实路由 handler**（`apply(ctx, Config(...))` + 假 webServer），
stub ssh 用 `/bin/sh` 真跑生成的命令，打在临时目录搭的假远端树上：
正控、硬链接 409、符号链接 404、sentinel 零泄漏、单次远端执行、40000 字节截断。

否证：退回 `| head -c 40000`，硬链接与符号链接两条变红（正控仍绿——证明修复不是「一律拒绝」）。

### 8.6 裸 `--write-floors` 被静默忽略（我自己踩的）

`opt()` 只认 `--name=value`，裸标志落回 `null`，写基线那段直接不执行，
**且输出与不加时逐字相同**。我跑完 `--write-floors` 看到全绿，以为下限已随新增测试上调，
实际文件一字未动，**差点带着过期下限交付**。过期下限不会让门变红，只会让它**不再拦人**——
正是本轮在追的那类假绿，而且这次是验收器自己的工具让我产生了错觉。

改成在**参数解析处**立即退 78（放在写盘处会先把整套门跑完、打完「退出码 = 0」再补报错，
读者容易只看见前一句，危害与静默忽略同类）。负控 NC19c 兼钉「必须在跑任何闸之前退出」。

否证：删掉该检查，NC19c 变红（且它是唯一一条会红的——正好说明此前无人守）。
