...
**在交付提交上复跑过一遍。** 上表的日志抓在 `d60ae14`；文档落地后我在
`3a3ba7a`（即交付树的最终形态）重跑了同一条命令，退出码 0，逐闸数字与上表**逐格相同**
（1054 / 0 / 8，tests 1062）。这条复跑是为了排除「数字量在某个中间状态、交付出去的树其实过不了」
这种情形——它不是新证据，是同一证据在正确的树上的重取。
