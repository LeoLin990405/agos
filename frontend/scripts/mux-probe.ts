...
import { createProbeState, recordFrame, recordReconnect, snapshot } from '../src/lib/mux-probe-model.ts'
...
const flush = (): void => {
  ...
  writeFileSync(OUT, JSON.stringify({ ...snapshot(state, Date.now()), base: BASE, opens: opened }, null, 2))
}

const stop = watchStream(
  (signal, onOpen) => client.mux(signal, () => { opened += 1; onOpen?.() }),
  (frame) => { state = recordFrame(state, { type: frame.payload.type, rpcId: String(frame.rpcId) }, Date.now()) },
