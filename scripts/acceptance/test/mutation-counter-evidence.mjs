...
const MUTATIONS = [
  ...
  {
    // 注意:必须把 `throw err` 一起吃掉才是忠实的 fail-open。第一版只改了 die() 那行,
    // 控制流会落到下面的 throw,进程照样非零退出 —— 看起来"被抓到了",其实抓到的是
    // 未捕获异常,不是 fail-open。那种突变会假装成功,掩盖真正的漏洞。
    id: 'M16',
    why: '【fail-closed】清单读不了时静默当成空清单继续跑',
    file: 'scripts/acceptance/prepare-host-modules.mjs',
    from: "\tif (err instanceof ManifestError) die(2, `宿主集成清单不可用(fail-closed,不降级成空清单):\\n  ${err.message.split('\\n').join('\\n  ')}`)\n\tthrow err",
    to: "\thostManifest = { path: '(none)', integrations: [] } // MUTATED: 静默空清单",
    suite: 'H',
  ...
  // M17 / M17b / M17c 是一组:阻断宿主集成违规**有两道彼此独立的闸门** ——
  // (a) exempt 只收 verdict==='absent-expected';(b) report.ready 单独 && hostIntegrationsOk。
  // 只拆其中一道,行为不变(下面两条 expectRed 为空、全绿就是证据);两道一起拆才漏。
  // 这组的价值在于:如果哪天有人"顺手简化"掉其中一道,绿色输出不会给任何提示,
  // 而这三条突变会把"现在只剩一道闸门了"这件事变成可见的事实。
  {
    id: 'M17',
    why: '【闸门 a】不看 verdict,清单里列了就一律豁免(只拆第一道)',
    file: 'scripts/acceptance/prepare-host-modules.mjs',
    ...
    suite: 'H',
    expectRed: [],
    expectStayGreen: ['NEG-HI-C1', 'NEG-HI-C2', 'NEG-HI-C3', 'NEG-HI-C4', 'NEG-HI-C5', 'NEG-HI-C6', 'NEG-HI-C8', 'NEG-HI-D1', 'NEG-HI-D2', 'NEG-HI-D3', 'NEG-HI-A1'],
    note: '只拆闸门 a 不漏 —— hostIntegrationsOk 这道独立顶住了。',
  },
  {
    id: 'M17b',
    why: '【闸门 b】把 hostIntegrationsOk 从就绪判定里摘掉(只拆第二道)',
    file: 'scripts/acceptance/prepare-host-modules.mjs',
    from: 'report.ready = resolvableAll && pinsOk && graphOk && hostIntegrationsOk',
    to: 'report.ready = resolvableAll && pinsOk && graphOk // MUTATED: 摘掉宿主集成闸门',
    suite: 'H',
    expectRed: [],
    expectStayGreen: ['NEG-HI-C1', 'NEG-HI-C2', 'NEG-HI-C3', 'NEG-HI-C4', 'NEG-HI-C5', 'NEG-HI-C6', 'NEG-HI-C8', 'NEG-HI-D1', 'NEG-HI-D2', 'NEG-HI-D3'],
    note: '只拆闸门 b 也不漏 —— exempt 不收 violation,那些包照旧算缺失。',
  },
  {
    id: 'M17c',
    why: '【两道一起拆】exempt 无条件放行 + 摘掉 hostIntegrationsOk —— 安全阀彻底失效',
    file: 'scripts/acceptance/prepare-host-modules.mjs',
    edits: [
      {
        from: "\tconst exempt = new Set(report.hostIntegrations.filter((h) => h.verdict === 'absent-expected').map((h) => h.specifier))",
        to: '\tconst exempt = new Set(report.hostIntegrations.map((h) => h.specifier)) // MUTATED',
      },
      {
        from: 'report.ready = resolvableAll && pinsOk && graphOk && hostIntegrationsOk',
        to: 'report.ready = resolvableAll && pinsOk && graphOk // MUTATED',
      },
    ],
    suite: 'H',
    ...
    note: '两道都拆才漏,且漏的时候十条负例全红 —— 说明它们确实各自在守,且清单外的默认行为不受牵连。',
  },
  {
    id: 'M18',
  ...
  {
    id: 'M19',
    why: '【报告可见性】把宿主集成专节的标题行删掉',
    file: 'scripts/acceptance/prepare-host-modules.mjs',
    from: "console.log(`-- 宿主环境集成(清单:${report.hostIntegrationsFile};缺席属预期,但不豁免安全阀) --`)",
    to: '// MUTATED: 专节标题消失',
    suite: 'H',
    expectRed: ['POS-HI-B1'],
    // B4 断言的是"清单为空"那行,不是标题行 —— 它保持绿正说明两条断言各管各的,
    // 没有一条断言在替另一条兜底。
    expectStayGreen: ['POS-HI-B4', 'NEG-HI-C1'],
    note: '「不能从输出里静默消失」这条要求确实被断言在守,不是写在注释里的空话。',
  },
  {
    id: 'M19b',
    why: '【报告可见性】把每条集成的状态行删掉(集成本身从输出里消失)',
    file: 'scripts/acceptance/prepare-host-modules.mjs',
    from: '\tconsole.log(`   ${mark} ${h.specifier}: ${state}`)',
    to: '\t// MUTATED: 状态行消失',
    suite: 'H',
    expectRed: ['POS-HI-B1', 'POS-HI-B3'],
    expectStayGreen: ['POS-HI-B4'],
  },
  {
    id: 'M20',
