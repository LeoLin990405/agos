...
test('W19 闸 1 穿越:objects/ab/../../../outside/<hex> → outside-root(先于嗅探)', async () => {
  ...
  // 用字符串拼,不用 path.join(它会把 .. 词法折叠掉,被测形态就变了)
  const r = await resolveMediaPath(objDir + '/../../../../outside/' + HEX, rootReal)
  assert.equal(r.ok, false)
  assert.equal(r.reason, 'outside-root')
})
...
test('W19 闸 4 伪造 mediaType:&mediaType= 与嗅探不符 → media-type-mismatch;非图片内容 → bad-magic;目录 → not-file;白名单扩展名文件也校验期望值', async () => {
  ...
  // FIFO:stat 先于 open,不能挂住
  const HEX6 = 'e'.repeat(64)
  const dir6 = join(root, 'v1', 'objects', 'ee'); mkdirSync(dir6, { recursive: true })
  const { execFileSync } = await import('node:child_process')
  execFileSync('mkfifo', [join(dir6, HEX6)])
  const t0 = Date.now()
  const r5 = await Promise.race([resolveMediaPath(join(dir6, HEX6), rootReal), new Promise((res) => setTimeout(() => res({ ok: false, reason: 'HUNG' }), 2000))])
  assert.equal(r5.reason, 'not-file', `FIFO 必须在 stat 处拒,不能 open(耗时 ${Date.now() - t0}ms)`)
